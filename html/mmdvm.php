...
<header class="ctrl-header">
  ...
  <button onclick="openRestore()" class="btn-header cyan">
    📂 Restore Configs
  </button>
...
function showActive(d) {
  ...
  document.getElementById('nxCenter').innerHTML = `<div class="nx-callsign">${esc(d.callsign)}</div>` + (d.name ? `<div class="nx-name">${esc(d.name)}</div>` : '');
}

function showYSFIdle() {
  ysfCurrentlyActive = false; animateVU(false, 'ysf');
  document.getElementById('ysfTxBar').className = 'nx-txbar';
  document.getElementById('ysfDest').textContent = '—'; document.getElementById('ysfProto').textContent = 'YSF';
...
function showYSFActive(d) {
  ...
  document.getElementById('ysfNxCenter').innerHTML = `<div class="nx-callsign ysf">${esc(d.callsign)}</div>` + (d.name ? `<div class="nx-name ysf">${esc(d.name)}</div>` : '');
}
...
function renderLastHeard(list, activeCall) {
  ...
  body.innerHTML = list.map(r => {
    ...
    return `<div class="lh-row${isActive?' lh-active':''}"><div class="lh-call-wrap">${dot}<span class="lh-call">${esc(r.callsign)}</span></div><span class="lh-name">${esc(r.name||'—')}</span><span class="lh-tg">${esc(r.tg||'—')}</span><span class="lh-time">${esc(r.time||'—')}</span><span class="lh-src ${srcCls}">${srcLbl}</span></div>`;
  }).join('');
}
...
function renderYSFLastHeard(list, activeCall) {
  ...
  body.innerHTML = list.map(r => {
    ...
    return `<div class="lh-row-ysf${isActive?' lh-active':''}"><div class="lh-call-wrap">${dot}<span class="lh-call-ysf">${esc(r.callsign)}</span></div><span class="lh-name">${esc(r.name||'—')}</span><span class="lh-time">${esc(r.time||'—')}</span><span class="lh-src-ysf ${srcCls}">${srcLbl}</span></div>`;
  }).join('');
}

async function fetchTransmission() {
  try { const r = await fetch('?action=transmission'); const d = await r.json(); if (d.active) showActive(d); else showIdle(); renderLastHeard(d.lastHeard||[], d.active?d.callsign:null); } catch(e) { showIdle(); }
}
async function fetchYSFTransmission() {
  try { const r = await fetch('?action=ysf-transmission'); const d = await r.json(); if (d.active) showYSFActive(d); else showYSFIdle(); renderYSFLastHeard(d.lastHeard||[], d.active?d.callsign:null); } catch(e) { showYSFIdle(); }
}
...
async function checkStatus() {
  try {
    ...
    running = gw || mmd; updateDMRButton(running);
  } catch(e) {}
}
async function checkYSFStatus() {
  try { const r = await fetch('?action=ysf-status'); const d = await r.json(); ysfRunning = d.ysf==='active'; setDot('dot-ysf', ysfRunning?'active':'off'); refreshYSFButton(); } catch(e) {}
}
async function checkMMDVMYSFStatus() {
  try { const r = await fetch('?action=mmdvmysf-status'); const d = await r.json(); mmdvmYsfRunning = d.mmdvmysf==='active'; setDot('dot-mmdvmysf', mmdvmYsfRunning?'active':'off'); refreshYSFButton(); } catch(e) {}
}
...
function refreshYSFButton() {
  const on = ysfRunning || mmdvmYsfRunning;
  document.getElementById('btnYSF').classList.toggle('running',on);
  document.getElementById('btnYSFLabel').textContent = on?'Cerrar C4FM':'Abrir C4FM';
  document.getElementById('ysfRefreshBadge').style.display = on?'flex':'none';
}

async function toggleServices() {
  const btn = document.getElementById('btnToggle'); const wasOpen = running;
  btn.classList.add('busy'); document.getElementById('btnLabel').textContent = wasOpen?'Deteniendo…':'Iniciando…';
  try {
    await fetch(wasOpen?'?action=stop':'?action=start');
    await new Promise(r => setTimeout(r,2200)); await checkStatus();
    if (wasOpen) { stopRefresh(); clearLog('logGw'); clearLog('logMmd'); showIdle(); document.getElementById('lhBody').innerHTML='<div class="lh-empty">Sin actividad reciente</div>'; }
    else startRefresh();
  } finally { btn.classList.remove('busy'); }
}

async function toggleYSF() {
  const btn = document.getElementById('btnYSF'); const wasOpen = ysfRunning || mmdvmYsfRunning;
  btn.classList.add('busy'); document.getElementById('btnYSFLabel').textContent = wasOpen?'Deteniendo…':'Iniciando…';
  try {
    if (wasOpen) {
      ...
      clearLog('logYsf'); clearLog('logMmdvmYsf'); stopYSFLogs(); stopMMDVMYSFLogs();
      showYSFIdle(); document.getElementById('ysfLhBody').innerHTML='<div class="lh-empty">Sin actividad C4FM</div>';
    } else {
...
async function rebootPi() {
  ...
  const btn = document.getElementById('btnReboot'); btn.textContent = '⏻ Reiniciando…'; btn.disabled = true;
...
async function confirmarInstalacion() {
  ...
  try {
    ...
    btn.disabled = false;
    btn.onclick = function() { closeInstalar(); };
  } catch(e) {
...
function openRestore() {
  ...
  msg.style.display = 'none'; msg.className = 'restore-msg';
}
...
async function doRestore() {
  ...
  const msg = document.getElementById('restoreMsg');
  msg.className = 'restore-msg loading'; msg.style.display = 'block'; msg.textContent = '⏳ Restaurando…';
  try {
    ...
    msg.textContent = '✖ Error en la solicitud.';
  }
}
...
async function fetchLogs() {
  try {
    const r = await fetch('?action=logs&lines=15'); const d = await r.json();
...
async function fetchYSFLogs() {
  try { const r = await fetch('?action=ysf-logs&lines=15'); const d = await r.json(); const el = document.getElementById('logYsf'); const atBot = el.scrollHeight-el.clientHeight<=el.scrollTop+10; el.innerHTML=colorize(d.ysf); if(atBot)el.scrollTop=el.scrollHeight; } catch(e) {}
}
async function fetchMMDVMYSFLogs() {
  try { const r = await fetch('?action=mmdvmysf-logs&lines=15'); const d = await r.json(); const el = document.getElementById('logMmdvmYsf'); const atBot = el.scrollHeight-el.clientHeight<=el.scrollTop+10; el.innerHTML=colorize(d.mmdvmysf); if(atBot)el.scrollTop=el.scrollHeight; } catch(e) {}
}
...
(async () => {
  ...
  if (running) startRefresh(); else { showIdle(); fetchTransmission(); }
